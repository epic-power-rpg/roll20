    ...
    getAttrs(disciplineinfo_names.concat(expertise_names)
      ...
      for (let i = 0; i < ids.length; ++i) {
        if (values[disciplineinfo_names[i]] === 'D') {
          under_attack_discipline = values[names_names[i].toLowerCase().trim() === 'attack';
        } else {
          if (values[disciplineinfo_names[i]] === '⇡') {
      ...
      let skill_map = _.object(
        names_names.map(name => values[name].toLowerCase().trim()),
        _.range(ids.length));
      let min_expertises = {};
